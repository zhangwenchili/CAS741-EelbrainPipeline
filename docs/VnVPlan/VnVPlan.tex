...
\midrule
    ...
    Dr. Spencer Smith & Course Instructor & Provides feedback on V\&V plan \\
...
This section defines the system-level tests derived from the Software Requirements Specification (SRS). It first presents test cases for functional requirements, followed by tests addressing nonfunctional requirements such as usability, maintainability, and portability. Finally, a traceability matrix is provided to explicitly link requirements to their corresponding test cases.
...
\item{FR-01\\}
    ...
    Input:
        \begin{itemize}
            \item A valid BIDS dataset containing multiple file paths (e.g., raw data files, event files, MRI files if applicable);
...
\item{FR-03\\}
    ...
    Test Case Derivation: R3 and R4 \\
    ...
    \begin{itemize}
        ...
        \item The dimensions of processed sensor-space data match expected values (the number of channels, time points, or events/epochs).
    \end{itemize}
...
\item{FR-04\\}
    ...
    \begin{itemize}
        \item The returned source-space data are in correct MNE-Python data structures (e.g., SourceEstimate objects);
        \item The dimensions of processed source-space data show some properties of correctness (e.g., has vertices/time points; not all zeros; metadata present).
    \end{itemize}
...
\item{NFR-01\\}
    ...
    Input/Condition: Participants complete a short task set (e.g., load raw files, run a sensor-space step, access results) and then complete a usability survey questionnaire. \\
    Output/Result: At least 80\% of respondents rate the software as easy to use (as defined by the survey threshold). \\
...
\item{NFR-02\\}
    Type: Dynamic, Manual \\
    Initial State: Baseline development time is recorded for the original implementation; codebase is under version control with issues/tasks. \\
    Input/Condition: Select one likely change (e.g., adding support for more BIDS entities or more preprocessing operations) and implement it by a developer with relevant domain knowledge. \\
    Output/Result: The measured effort (person-hours) to implement the chosen likely change is less than 0.5 of the original development time. \\
...
\item{NFR-03\\}
    ...
    Input/Condition: Install Eelbrain pipeline and run a minimal workflow on each OS using the same small BIDS test dataset. \\
    ...
    How test will be performed: Manually set up and execute the testing.
